...
- Open another can device as the data sender (it can be a pc can, or another can device on the development board; here another can device is used to send data, PC can can be verified by yourself)
...
- When debugging rcan on the MUSE-Pi development board, you need to disable the following dts pin configuration
