...
1. 进入 OTA 打包工具目录，并创建 `target_package`、`output_package` 和 `sign_cert` 文件夹，如果已经存在文件夹，就不需要创建
...
1. `sign_cert` 文件夹中放入公钥，公钥来自

```c
base/update/updater/test/unittest/test_data/src/signing_cert.crt
```

1. `target_packet` 文件夹中放入要打包生成 OTA 全量包的文件，包括：
   1. 私钥，rsa\_private\_key2048.pem，来自
   ...
   1. 二进制升级文件，`updater_binary`，来自

   ```c
   out\xxx\packages\phone\system\bin\updater_binary
   ```

   1. 升级组件配置文件夹 `updater_config`，该文件夹内部包括：
      1. 升级包支持的产品列表：BOARD.list，来自
      ...
      1. 组件配置文件：updater\_specified\_config.xml，来自

      ```c
      device/board/hisilicon/hispark_taurus/linux/system/updater_specified_config.xml
      ```

      - 修改 `fileVersion` 为 2（4.0 更新的校验方式）
      - `softVersion` 改为当前软件版本
      - `compType` 改为 0,表示全量(1 表示增量)
      - 修改 img 打包路径，`/vendor.img` 和 `./system.img` 等
        最终内容如下：
...
1. 全量包打包，会在 `base/update/packaging_tools/output_package` 目录生成 `updater_full.zip` 全量包，命令如下：
...
1. 创建 lib 文件夹,放入差分镜像二进制工具，如果已经存在这些文件，则忽略这步
...
1. packaging\_tools 目录下，拷贝上一次 target\_package 作为制作差分包的 source\_package

```shell
fuqiang@snode2:~/WorkSpace/oh5/base/update/packaging_tools$ cp -rf target_package/* source_package/
```

1. 更新 target\_package 目录中的文件

   1. 替换成新的镜像文件
   2. 替换成新的二进制升级文件
   3. 更新 `target_package/updater_config/updater_specified_config.xml` 的 `softVersion` 和 `compType`，修改 softVersion 需大于当前系统版本； compType 设为 1，表示增量，部分小的 img 的 compType 还是设置为 0，使用全量升级
...
将制作好的全量包 updater\_full.zip（或者差分包 updater\_diff.zip）放入设备，设置 reboot 模式
...
OH5.0 上面遇到生成的二进制 updater\_binary 无法执行，执行会触发 signal11，经过和 OH4.0 对比，发现 OH5.0 没有把 updater\_binary 定义为 executable，怀疑这里存在问题，按照 OH4.0 修改即可，补丁如下：
...
引用自 [进迭时空开发者文档](https://developer.spacemit.com/documentation?token=TYDVwpqfYiufN4kokUec4VtQn4d)